...
Xp_true = A0*Xn + B0*U;
epsilon = 0.05;
eps_noise = epsilon*(2*rand(n, T)-1);
Xp = eps_noise + Xp_true;
...
Xp_curr = A*Xn + B*U + dx;
Xp_sub = replace(Xp_curr, [A(:); B(:); dx(:)], [A0(:); B0(:); eps_noise(:)]);
...
cons = [ss_con:'superstability'; dd_con:'buckets'; rob_con:'robust epsilon'; uncertain(rob_vars_all)];
